...
# Little Unclear, Need to do it again.

class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        # Setting two sets; one for storing results the other for storing duplicates
        res, dups = set(), set()
		# Enumerating nums list and searching for all values of it that sum to 0
        for i, val1 in enumerate(nums):
            ...
            if val1 not in dups:
				...
				# and if it does it means we can have triplets that sum to zero
				# and add this to res set as a possible solution
				# return the tuples of values that sum to zero
